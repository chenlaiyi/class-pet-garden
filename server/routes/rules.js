...
function copyDefaultRules(userId) {
  ...
  const insertMany = db.transaction((rules) => {
    for (const rule of rules) stmt.run(uuidv4(), rule.name, rule.points, rule.category, userId, now)
  })
...
router.get('/', authMiddleware, requireNotUser, (req, res) => {
  ...
  if (rules.length === 0) {
    ...
    rules = listRulesForUser(req.userId)
  }
  res.json({ rules })
})

// 添加规则 — 仅老师/super_admin
router.post('/', authMiddleware, requireAtLeastTeacher, (req, res) => {
  const { name, points, category } = req.body
  if (!name || !name.trim()) return res.status(400).json({ error: '规则名称不能为空' })
  const id = uuidv4()
  const now = Date.now()
  db.prepare(`INSERT INTO evaluation_rules (id, name, points, category, user_id, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
    .run(id, name.trim(), points || 0, category || '其他', req.userId, now)
  res.json({ id, name: name.trim(), points: points || 0, category: category || '其他', user_id: req.userId, created_at: now })
})

// 重置为默认规则 — 仅老师/super_admin
router.post('/reset', authMiddleware, requireAtLeastTeacher, (req, res) => {
  db.prepare('DELETE FROM evaluation_rules WHERE user_id = ?').run(req.userId)
  copyDefaultRules(req.userId)
  const rules = listRulesForUser(req.userId)
  res.json({ success: true, rules })
})

// 更新规则 — 仅拥有者/super_admin
router.put('/:id', authMiddleware, requireAtLeastTeacher, (req, res) => {
  const { name, points, category } = req.body
  const rule = verifyRuleOwnership(req.params.id, req.userId)
  if (!rule) return res.status(404).json({ error: '规则不存在或无权修改' })
  db.prepare(`UPDATE evaluation_rules SET name = ?, points = ?, category = ? WHERE id = ?`)
    .run(name?.trim() || '', points || 0, category || '其他', req.params.id)
  res.json({ success: true })
})

// 删除规则 — 仅拥有者/super_admin
router.delete('/:id', authMiddleware, requireAtLeastTeacher, (req, res) => {
  const rule = verifyRuleOwnership(req.params.id, req.userId)
  if (!rule) return res.status(404).json({ error: '规则不存在或无权删除' })
  db.prepare('DELETE FROM evaluation_rules WHERE id = ?').run(req.params.id)
  res.json({ success: true })
})
...
router.get('/frequent', authMiddleware, requireNotUser, (req, res) => {
  ...
  const frequentRules = db.prepare(`
    ...
    WHERE er.user_id = ?
    AND EXISTS (SELECT 1 FROM evaluation_rules r WHERE r.user_id = ? AND r.name = er.reason AND r.points = er.points)
    GROUP BY er.reason, er.points, er.category
    ORDER BY CASE WHEN er.points > 0 THEN 0 ELSE 1 END, use_count DESC
    LIMIT 25
  `).all(userId, userId)
